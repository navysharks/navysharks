...
interface TransportModalProps {
  ...
  onClose: () => void;
  standardQty: number;
  bulletProofQty: number;
  onSetStandardQty: (qty: number) => void;
  onSetBulletProofQty: (qty: number) => void;
  onAddToCart: () => void;
}
...
export function TransportModal({
  ...
  const total = (standardQty * 499) + (bulletProofQty * 799);
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in">
      <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="space-y-4">
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  className="accent-cyan-400 w-5 h-5"
                  checked={bulletProofQty > 0}
                  onChange={() => onSetBulletProofQty(bulletProofQty > 0 ? 0 : 1)}
                />
              ...
              {bulletProofQty > 0 && (
                ...
                >
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetBulletProofQty(Math.max(1, bulletProofQty - 1)); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >-</button>
                  <span className="text-base w-6 text-center font-bold text-white">{bulletProofQty}</span>
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetBulletProofQty(bulletProofQty + 1); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >+</button>
                </div>
              )}
          ...
          {(standardQty > 0 || bulletProofQty > 0) && (
            <div className="pt-4">
              <button
