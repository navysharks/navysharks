...
interface MarineModalProps {
  isOpen: boolean;
  onClose: () => void;
  standardQty: number;
  upgradeQty: number;
  onSetStandardQty: (qty: number) => void;
  onSetUpgradeQty: (qty: number) => void;
  onAddToCart: () => void;
}

export function MarineModal({
  isOpen,
  onClose,
  standardQty,
  upgradeQty,
  ...
  const total = (standardQty * 499) + (upgradeQty * 899);
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  className="accent-cyan-400 w-5 h-5"
                  checked={upgradeQty > 0}
                  onChange={() => onSetUpgradeQty(upgradeQty > 0 ? 0 : 1)}
                />
              ...
              {upgradeQty > 0 && (
                ...
                >
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetUpgradeQty(Math.max(1, upgradeQty - 1)); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >-</button>
                  <span className="text-base w-6 text-center font-bold text-white">{upgradeQty}</span>
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetUpgradeQty(upgradeQty + 1); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >+</button>
                </div>
              )}
