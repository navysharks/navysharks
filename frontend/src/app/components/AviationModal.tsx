...
interface AviationModalProps {
  ...
  onClose: () => void;
  standardQty: number;
  firstClassQty: number;
  onSetStandardQty: (qty: number) => void;
  onSetFirstClassQty: (qty: number) => void;
  onAddToCart: () => void;
}

export function AviationModal({
  isOpen,
  onClose,
  standardQty,
  firstClassQty,
  ...
  const total = (standardQty * 349) + (firstClassQty * 999);
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  className="accent-cyan-400 w-5 h-5"
                  checked={firstClassQty > 0}
                  onChange={() => onSetFirstClassQty(firstClassQty > 0 ? 0 : 1)}
                />
              ...
              {firstClassQty > 0 && (
                ...
                >
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetFirstClassQty(Math.max(1, firstClassQty - 1)); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >-</button>
                  <span className="text-base w-6 text-center font-bold text-white">{firstClassQty}</span>
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetFirstClassQty(firstClassQty + 1); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >+</button>
                </div>
              )}
