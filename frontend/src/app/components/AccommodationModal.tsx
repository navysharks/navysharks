...
interface AccommodationModalProps {
  ...
  onClose: () => void;
  standardQty: number;
  highRollerQty: number;
  onSetStandardQty: (qty: number) => void;
  onSetHighRollerQty: (qty: number) => void;
  onAddToCart: () => void;
}
...
export function AccommodationModal({
  ...
  const total = (standardQty * 249) + (highRollerQty * 749);
            ...
            >
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  className="accent-cyan-400 w-5 h-5"
                  checked={highRollerQty > 0}
                  onChange={() => onSetHighRollerQty(highRollerQty > 0 ? 0 : 1)}
                />
              ...
              {highRollerQty > 0 && (
                ...
                >
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetHighRollerQty(Math.max(1, highRollerQty - 1)); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >-</button>
                  <span className="text-base w-6 text-center font-bold text-white">{highRollerQty}</span>
                  <button
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onSetHighRollerQty(highRollerQty + 1); }}
                    className="w-8 h-8 flex items-center justify-center hover:bg-slate-700 rounded-md text-slate-300 transition-colors font-medium"
                  >+</button>
                </div>
              )}
