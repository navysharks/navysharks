...
export function Experiences() {
  ...
  useEffect(() => {
    ...
    if (sessionId && user) {
      const savedState = localStorage.getItem('pendingCheckoutState');
      ...
      if (savedState) {
        try {
          ...
          // Fire API directly with resolved values — no stale closure risk
          (async () => {
            if (!restoredBundleName) return;
  ...
  const handleBeforeRedirect = (modalAddons: string[]) => {
  ...
  const handleCheckoutComplete = async (checkoutModalAddons: string[] = [], guestDetails?: { firstName: string; lastName: string; email: string; phone: string }, verificationSessionId?: string) => {
    if (!user) {
